...
public class OrderUtil {
	...
	public static String buildOrderInfo(Map<String, String> req) {
		...
		buf.append("&seller_id=\""
				+ AlipayConfig.getConf(AlipayConfig.SELLER_ID) + "\"");
		// 商户网站唯一订单号
		...
		buf.append("&total_fee=\""
				+ (Long.parseLong(req.get(Fields.F_payAmount)) / 100.0) + "\"");
		// 服务器异步通知页面路径
		buf.append("&notify_url=\""
				+ AlipayConfig.getConf(AlipayConfig.NOTIFY_URL) + "\"");
		// 接口名称， 固定值
		buf.append("&service=\"" + AlipayConfig.getConf(AlipayConfig.SERVICE)
				+ "\"");
		// 支付类型， 固定值
		buf.append("&payment_type=\""
				+ AlipayConfig.getConf(AlipayConfig.PAYMENT_TYPE) + "\"");
		// 参数编码， 固定值
		buf.append("&_input_charset=\""
