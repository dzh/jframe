...
public class DefPluginContext implements PluginContext, Dispatchable {
	 ...
	 */
	public void regPlugins(Collection<Plugin> plugins,
		...
		while (iter.hasNext()) {
			...
			try {
				...
				_LOG.error("When invoke pluginContext.regPlugin(), plugin name is "
	 ...
	 * java.util.Comparator)
	 */
	public void unregPlugins(Collection<PluginRef> plugins,
			Comparator<? super Plugin> comparator) {
		if (plugins.size() == 0)
			return;
