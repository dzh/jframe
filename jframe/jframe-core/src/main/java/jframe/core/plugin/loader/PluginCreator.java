...
public class PluginCreator {
    ...
    public PluginCase loadPlugin(File plugin) {
        ...
        try {
            ...
            if (p.getProperty(PluginCase.P_PLUGIN_DLL) != null) {
                ...
                if (dlls.size() > 0) pc.setPluginDll(dlls);
            }
            if (p.getProperty(PluginCase.P_PLUGIN_LIB) != null) {
                ...
                if (libs.size() > 0) pc.setPluginLib(libs);
            }
    ...
    protected List<String> parseList(String pValue) {
        if (pValue == null || "".equals(pValue)) return Collections.emptyList();
    ...
    public Plugin createPlugin(PluginClassLoader pcl, PluginCase pc) {
        if (pcl == null || pc == null) return null;
        return pcl.getPlugin();
    }
    ...
    public boolean isValidPlugin(JarFile jar) {
        ...
        try {
            ...
            Properties p = new Properties();
            p.load(is);
            if (p.getProperty(PluginCase.P_PLUGIN_CLASS) == null || p.getProperty(PluginCase.P_PLUGIN_NAME) == null) { return false; }
        } catch (IOException e) {
            LOG.error(e.getMessage());
        } finally {
            if (is != null) try {
